  ...
  (call/cc
   (lambda (return)
     (case (car struct)
       ...
       ((fetch) (let ((type (pop-current)))
                  (if (not (type= '(addr byte) type))
                      (error "tos was " type " when @ was called")
                      (current+ 'byte))))
       ((bang) (let* ((addr (pop-current))
                      (val (pop-current)))
                 (if (or (not (type= '(addr any) addr))
                         (not (type= val (referenced-type addr))))
                     (error "setting " addr " with " val
                            " via ! operation"))))
       ((store) (let* ((addr (pop-current))
                       (val (pop-current)))
                (if (or (not (type= '(addr byte) addr))
                        (not (type= 'byte val)))
                    (error "setting " addr " with " val
                           " via store operation"))))
       ...
       ((> < >= <=) (let* ((tos (pop-current))
                           (nos (pop-current)))
                      (cond ((not (type= tos nos))
                             (error "types " tos " and " nos " do not match"))
                            ((or (type= tos '(ptr any))
                                 (type= nos '(ptr any)))
                             (error "relational op on " tos " and " nos))
                            (else (current+ 'bool)))))
...
;; TODO: that's mostly a copy of the arm code
(define (gen-x86-64 what . args)
  (case what
    ((enter-if enter-eif)
     (emit "cmp r0, #0")
     (emit "pop r0")
...
(define gen gen-x86-64)

;; How to use: call (parse-toplevel) until eof, then (compile).

;; TODO:
;> (parse-toplevel)
;def baz (bool int :: bool) [drop]
;declared effect (bool) does not match actual effect (int)
