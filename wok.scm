...
(define (compile-eif then-branch else-branch)
  (let* ((middle (genlabel))
         (end (genlabel)))
    (gen 'enter-eif middle)
    (compile-block then-branch)
    (gen 'enter-else-for-if end middle)
...
(define (compile-ehas then-branch else-branch)
  (let* ((middle (genlabel))
         (end (genlabel)))
    (gen 'enter-ehas middle)
    (compile-block then-branch)
    (gen 'enter-else-for-has end middle)
    (compile-block else-branch)
    (gen 'exit-ehas end)))

(define (compile-loop body)
  (let ((start (genlabel)))
    (gen 'enter-loop start)
    (compile-block body)
    (gen 'exit-loop start)))
...
(define (gen-arm what . args)
  (case what
    ((enter-if enter-eif)
     ...
     (emit "pop r0")
     (emit "be .L" (car args)))
    ((exit-if exit-eif exit-has exit-ehas)
     (emit ".L" (car args) ":"))
    ((enter-else-for-if)
     (emit "b .L" (car args))
     (emit ".L" (cadr args)  ":"))
    ((enter-has)
     (emit "cmp r0, #0")
     (emit "pope r0") ; different from `if`
     (emit "be .L" (car args)))
    ((enter-ehas)
     (emit "cmp r0, #0")
     (emit "be .L" (car args)))
    ...
    ((break)
     (emit "b .L" (top-loop)))
    ((stop exit-def)
     (emit "ldr pc, r12 + #4")) ;; TODO: fix this
    ((enter-def)
     (emit (obj-symbol (car args)) ":")
     (emit "str lr, r12, #-4")) ;; TODO: fix this
    ((at)
     (emit "ldr r0, r0"))
    ((set)
     (emit "pop r1")
    ...
    ((drop)
     (emit "pop r0"))
    ((dropem)
     (emit "TODO")) ; TODO: can do in 1 instruction?
    ((nip)
     (emit "add sp, #4"))
    ((x)
     (emit "ldr r1, sp")
